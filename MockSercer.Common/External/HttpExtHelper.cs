...
{
    public static class HttpExtHelper
    {
        /// <summary>
        ...
        /// <summary>
        /// Post将对象转为Xml格式发送到指定Url
        /// </summary>
        /// <param name="obj"></param>
        /// <param name="requestUrl"></param>
        /// <returns></returns>
        public static async Task<string> PostAsXmlAsync(this object obj, string requestUrl)
        {
            HttpClient client = new HttpClient();
            string responseString = string.Empty;
            HttpContent content = new StringContent(obj.ToXml(), Encoding.UTF8, "application/xml");
            HttpResponseMessage response = await client.PostAsync(requestUrl, content);
            if (response.StatusCode == HttpStatusCode.OK)
            {
                responseString = await response.Content.ReadAsStringAsync();
